<script lang="ts">
	...
	const LEVELS: FilterLevel[] = [
		{
			key: 'trial',
			endpoint: '/trials',
			valueField: 'trialDbId',
			labelField: 'trialName',
			paramName: 'trialDbId',
			label: 'Trial',
			plural: 'trials'
		},
		{
			key: 'study',
			endpoint: '/studies',
			valueField: 'studyDbId',
			labelField: 'studyName',
			paramName: 'studyDbId',
			label: 'Study',
			plural: 'studies'
		},
		{
			key: 'germplasm',
			endpoint: '/germplasm',
			valueField: 'germplasmDbId',
			labelField: 'germplasmName',
			paramName: 'germplasmDbId',
			label: 'Germplasm',
			plural: 'germplasm',
			virtualList: true
		},
		{
			...
			valueField: 'observationUnitDbId',
			labelField: 'observationUnitName',
			paramName: 'observationUnitDbId',
			label: 'Observation Unit',
			plural: 'observation units',
			virtualList: true
		}
	];
	...
	async function requestBrapi(
		...
		try {
			...
			const response = await fetch(
				baseUrl + endpoint + (searchParams?.toString() ? `?${searchParams.toString()}` : ''),
				{
					headers: {
	 ...
	 */
	async function requestBrapiPages(
		endpoint: string,
		...
		while (true) {
			const response = await requestBrapi(endpoint, {
				...parameters,
				page: `${page}`,
				pageSize: `${pageSize}`
			});
	...
	async function loadLevel(i: number) {
		...
		const params: Record<string, string> =
			prev && selected[prev.key] ? { [prev.paramName]: selected[prev.key]! } : {};
		const { data, truncated: wasTruncated } = await requestBrapiPages(
			level.endpoint,
			params,
			MAX_LIST_PAGES
		);
	...
	async function testConnection() {
		...
		if (response) {
			connected = true;
			imagesSupported = !!response.result?.calls?.some(
				(call: Call) => call.service === 'images' && call.methods?.includes('GET')
			);
		} else {
			connected = false;
...
<fieldset class="fieldset">
	...
	>
			...
			<table class="table">
				...
				<tbody>
					{#each files as file, i (i)}
						{#if file.comments.find((c: { name: string }) => c.name === 'mimeType') && file.comments
								.find((c: { name: string }) => c.name === 'mimeType')
							...
							<tr>
								...
								<td>
									<Select
										...
										options={[
											...
											{
												label: 'Acquisition Parameter Data File',
												value: 'Acquisition Parameter Data File'
											},
											...
											{
												label: 'Post Translational Modification Assignment File',
												value: 'Post Translational Modification Assignment File'
											},
											{
												label: 'Derived Array Data Matrix File',
												value: 'Derived Array Data Matrix File'
											},
											{
												label: 'Free Induction Decay Data File',
												value: 'Free Induction Decay Data File'
											},
								...
								<td
									><span class="badge badge-outline badge-info"
										>{file.comments.find(
											(c: { name: string; value: string }) => c.name === 'mimeType'
										).value}</span
									></td
								>
								<td
									>{file.comments.find(
										(c: { name: string; value: string }) => c.name === 'imageTimeStamp'
									).value}</td
								>
								<td
									><button class="btn btn-outline btn-error btn-sm" onclick={() => removeImage(i)}
										>Remove</button
									></td
								>
							</tr>
		...
		{:else}
			<span class="label p-4 italic">No images imported yet</span>
		{/if}
	</section>
	<p class="label">You can import image metadata from the BrAPI Server</p>
	<Modal
		class="max-h-[85vh] w-11/12 max-w-5xl"
		label="Import Images"
		onopen={() => {}}
		onapprove={addImages}
	>
		...
		<fieldset class="fieldset w-full rounded-box border border-base-300 bg-base-200 p-4">
			...
			{#if connected !== undefined}
				<span class="label {connected ? 'text-success' : 'text-error'}"
					>{connected ? 'Connected successfully' : 'Connection failed'}</span
				>
				{#if connected && imagesSupported === false}
					<span class="label text-warning"
						>This server doesn't advertise BrAPI Images support in its /serverinfo response - the
						request below may still fail.</span
					>
				{/if}
			{/if}
		</fieldset>
		{#if connected}
			<fieldset class="fieldset w-full rounded-box border border-base-300 bg-base-200 p-4">
				...
				{#each LEVELS as level, i (level.key)}
					<section class="rounded-lg border" style={pickerBorderStyle}>
						<fieldset class="fieldset p-4">
							...
							{#if truncated[level.key]}
								<span class="label text-warning">
									Showing first {lists[level.key].length}
									{level.plural} only
									{#if i > 0}
										- select a {LEVELS[i - 1].label} above to narrow this down.{:else}
										- server has more.{/if}
								</span>
							{/if}
						</fieldset>
						...
						<tbody>
							{#each images as image}
								<tr>
									...
									<td><span class="badge badge-outline badge-info">{image.mimeType}</span></td>
